...
const normalizePath = function(req_options){
  ...
  const raw_data = {
    pathname: req_options.pathname,
    path:     (req_options.path || req_options.pathname),
    href:     req_options.href
  }

  if (!raw_data.pathname)
    return

  const normalized_pathname = path.posix.normalize(raw_data.pathname)

  if (normalized_pathname !== raw_data.pathname){
    req_options.pathname = normalized_pathname

    if (raw_data.path && (raw_data.path.indexOf(raw_data.pathname) >= 0))
      req_options.path = raw_data.path.replace(raw_data.pathname, normalized_pathname)

    if (raw_data.href && (raw_data.href.indexOf(raw_data.pathname) >= 0))
      req_options.href = raw_data.href.replace(raw_data.pathname, normalized_pathname)
  }
}
